...
contract POP is ERC721, ERC721URIStorage, KeeperCompatibleInterface {
    ...
    constructor(uint256 _interval) ERC721("POP Chainlink", "popcl") {
        interval = _interval;
    ...
    function POPCHANGE(uint256 _tokenId) public {
        if (POPCL(_tokenId) >= 2) {
            return;
        }

        uint256 newVal = POPCL(_tokenId) + 1;
        // store the new URI
    ...
    function POPCL(uint256 _tokenId) public view returns (uint256) {
